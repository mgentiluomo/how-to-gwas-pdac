...
The log reports what was actually analysed. On the demonstration data: **219 cases and 392
controls**.
...
On the demonstration data **λ = 1.016**, and λ₁₀₀₀ = 1.058.
...
| | |
|---|---|
| Variant | 9:133266804:G:T, at *ABO* (9q34.2) |
| Effect allele reported by PLINK | T, frequency 0.386 |
| Odds ratio as printed | 0.415 (95% CI 0.312 to 0.552) |
| Odds ratio per copy of the other allele | 2.41 (95% CI 1.81 to 3.20) |
| P | 1.4 × 10⁻⁹ |
| Variants at *P* < 5 × 10⁻⁸ | 11, all in this region |
...
::: {.callout-important title="The lead variant is not the causal variant"}
The phenotype was simulated from `9:133273682:A:T`. That variant is **not** the one at the top of
the scan. It sits 6.9 kb away, at *P* = 4.4 × 10⁻⁹ against the lead's 1.4 × 10⁻⁹, and its odds
ratio per copy of the risk allele is 2.34 (95% CI 1.76 to 3.10).

The two variants are in tight linkage disequilibrium and carry the same signal. Which of them
reaches the top is decided by sampling noise, and with eleven variants above genome-wide
significance in this region the one that wins is, by construction, the one whose estimate drifted
furthest. Selection is operating here, not on the size of the effect but on the identity of the
variant reported.

This is why a lead variant should be reported as the top of a credible set rather than as a
finding, and it is what
[Fine mapping](../06_finemapping_annotation/06_finemapping_annotation.qmd) exists to resolve. In
that section the causal variant ranks third among eleven in the European credible set, and first
once the ancestry strata are combined.
:::

::: {.callout-note title="Read the direction before reading the number"}
PLINK reports the effect of the allele in its `A1` column, which here is `T`. The risk allele is
the other one, the reference `A`, and at this locus it is the **major** allele. That is why the
printed odds ratio is 0.415 rather than something above 1. The reciprocal, 2.41, is the number to
compare with anything stated on the risk-allele scale.

Nothing is wrong with the output. The orientation is simply a property of how the tool reports,
and getting it backwards is one of the most common ways a real analysis goes quietly astray, which
is why [Meta-analysis](../05_meta_analysis/05_meta_analysis.qmd) treats allele alignment as a
first-order concern rather than a formatting detail.
:::

::: {.callout-note title="Why there is no winner's curse to show here"}
An underpowered scan overstates the effects it discovers, because only upward fluctuations cross
the threshold. It is worth stating plainly that **this did not happen at this locus**, and why.

The generative odds ratio is 2.60. The analysis recovers 2.34, slightly below rather than above.
At the 99.1% power reported in
[Statistical power](../03B_statistical_power/03B_statistical_power.qmd) there is no selection to
inflate anything: the variant was going to be detected whatever the noise did, so the estimate
simply scatters around the truth.

The rule generalises in the useful direction. The winner's curse is a consequence of low power
rather than a property of discovery, so the question to ask of any published effect size is what
power produced it. An estimate obtained at 2% power, as TERT/CLPTM1L would have been here, should
not be carried into risk prediction, into meta-analytic weighting or into a replication power
calculation. An estimate obtained at 99% power can be.
:::
