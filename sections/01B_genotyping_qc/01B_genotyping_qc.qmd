...
**Why.** Extremely high or low heterozygosity can signal contamination, inbreeding, ancestry
outliers, or technical artifacts. This step uses autosomal heterozygosity and removes samples
outside a standard deviation-based threshold.
...
In the demo run, this step removed **2 samples**.
...
**Why.** Strong Hardy-Weinberg equilibrium (HWE) deviations in controls often indicate genotyping
...
In the demo run this step removed **14,573 variants**, leaving **406,947 variants**.

![](results/pdac_demo_06_hwe_pvalue_distribution.png){fig-alt="Histogram of minus log10 Hardy-Weinberg p-values."}

::: {.callout-warning title="Do HWE in controls"}
For disease GWAS, avoid filtering on HWE calculated in all samples if cases are included. A real
disease association can create deviation in cases. This tutorial therefore performs the HWE filter
using controls.
:::
...
| Second degree | 4 | 8 |
| Third degree | 10 | 20 |
| No third-degree-or-closer pair | NA | 964 |
| Total samples before relatedness filter | NA | 1,430 |

Example pruning diagnostics:

| Metric | Value |
|---|---:|
| Samples before relatedness filter | 1,430 |
| KING pruning pairs > 0.1875 | 284 |
| Unique samples in KING pruning pairs | 448 |
| Relatedness components | 164 |
| Largest component samples | 6 |
| PLINK2 `--king-cutoff` removed samples | 167 |
| Phenotype-aware removed samples | 191 |
| Phenotype-aware removed percent | 13.4 |
| Controls removed | 154 |
| Cases removed | 37 |

Example KING vs PI_HAT summary:

| Metric | Value |
|---|---:|
| Total pairs in either report | 55,337 |
| Pairs reported by both | 298 |
| Same category | 294 |
| Different category, check values | 4 |
| Reported by KING only | 0 |
| Reported by PI_HAT only | 55,039 |
| Pruning pairs | 284 |
| Pruning pairs with PI_HAT | 284 |
| Mean absolute `PI_HAT - 2 * KING` | 0.025632 |
| Maximum absolute `PI_HAT - 2 * KING` | 0.126513 |

::: {.callout-warning title="Review large relatedness loss"}
In the demo run, phenotype-aware pruning removed **195 samples**. That is a substantial sample
...
In the demo run this step removed **6,820 variants**, leaving **414,695 variants** and
**1,217 samples**.
...
| Relatedness | 1,217 | 406,947 | 191 | 0 |
| MAF filter | 1,217 | 414,695 | 0 | 6,820 |

Final retention in the demo run was **83.3% of samples** and **96.4% of variants**.
