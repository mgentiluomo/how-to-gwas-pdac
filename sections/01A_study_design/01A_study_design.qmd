...
| GenCall distribution | bimodal, median 0.72, 1.1% below the 0.15 threshold |
...
| Ancestry of the study | An array designed largely on European haplotypes tags other populations less well on the directly genotyped backbone, and arrays differ in whether they carry population-specific content. This is a question about content, not about the coverage figure |
...
median is 0.72 and 1.1% of calls fall below the threshold. A further 1.3% are reported as missing
outright: the two are different failures, and the per-sample log below counts them separately.
...
These counts run on the 7,520 variants matched to the manifest, not on all 8,000, which is why
they sit slightly below the 25% and the 104 that were injected.
