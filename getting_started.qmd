...
### 5.3 Common Step 5 problems and fixes
...
| `Unable to locate package r-base` | Package lists are stale, `universe` is disabled, or the Ubuntu release is unusual | Run the R install commands below; for persistent problems, use an Ubuntu LTS WSL release |
...
If R is missing on WSL/Linux, first try the Ubuntu package:
...
If Ubuntu cannot find `r-base`, enable the `universe` repository and refresh the package list:
...
### 5.5 Advanced fallback: manual installation
...
#### 4. Install micromamba and REGENIE
...
R is used later for QC plots, so confirm it is available:

```bash
R --version
```

### 5.6 Ready for the setup test

The full setup test is the next step. If you want to check immediately, run:
